...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if OTHER is not None:
        X = OTHER['X']
        P = OTHER['P']
        last_xy = OTHER['last_xy']
        if 'last_heading' not in OTHER:
            heading = atan2(measurement[0] - last_xy[0], measurement[1] - last_xy[1])
            OTHER['last_heading'] = heading
            print "I have a last heading"
        else:
            print "step 2"
            last_xy = OTHER['last_xy']
            heading = atan2(measurement[0] - last_xy[0], measurement[1] - last_xy[1])
            turning_angle = heading - OTHER['last_heading']
            D = distance_between(last_xy, measurement)
            print D
            F = matrix([[1, 0, 0],
                        [-D*sin(heading+turning_angle), 1, 0],
                        [D*cos(heading+turning_angle), 0, 1]]) #jacobian of the next state function

            #if we have enough information, then go for it...
            #prediction update
            new_X = matrix([[]])
            new_X.zero(3, 1) #3 rows of zeros, 1 column
        ...
        P = matrix([[1000, 0, 0],
                    [0, 1000, 0],
                    [0, 0, 1000]]) #uncertainty of angle is low, certainty of posit
        #print "the last xy"
        #OTHER = {'last_xy': measurement}
    ...
    if OTHER is None:
        OTHER = {'X': X, 'P': P, 'last_xy': measurement, 'last_heading': heading}
    else:
        OTHER['X'] = X
        OTHER['P'] = P
        OTHER['last_xy'] = measurement
