...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if OTHER is None:
        OTHER = []
        OTHER.append([0.5, measurement[0], measurement[1]]) #deep copy w/ heading guess
        lastxy = measurement
        return [0,0], OTHER #return
    else:
        lastxy = OTHER[-1] #I wonder if this creates a weird reference issue b/c I append on the next step
        #calculate two vectors based on the last two data points
        x = measurement[0]
        y = measurement[1]
        dx = measurement[0] - OTHER[-1][1]
        dy = measurement[1] - OTHER[-1][2]
        heading = atan2(dy, dx)
        OTHER.append([heading, measurement[0], measurement[1]]) #store the information here
        step_size = sqrt(dx**2 + dy**2) #could also act as velocity

    #KALMAN IMPLEMENTATION

    # measurement update
    #measurements = OTHER[1:] #I skip the first one b/c I am lazy
    D = step_size
    alpha = OTHER[-1][0] - OTHER[-2][0] #calculate the difference between the last two headings
    #this might actually blow up
    R = matrix([[1e-20,1e-20], [1e-20,1e-20]]) #measurement uncertainty
    ...
    F = matrix([[1, 0, 0],
                [-D*sin(theta), 1, 0],
                [D*cos(theta), 0, 1]]) #jacobian
    H = matrix([[0, 1, 0],
                [0, 0, 1]]) #Jacobian of the measurement function
    P = matrix([[1000, 0, 0],
                [0, 0, 0],
                [0, 0, 0]]) #uncertainty of angle is low, certainty of position is high
    u = matrix([[0, 0, 0],
                [0, 0, 0],
                [0, 0, 0]]) #external motion
    I = matrix([[1, 0, 0],
                [0, 1, 0],
                [0, 0, 1]]) #identity matrix
    x = matrix([[OTHER[0][0]],
                [OTHER[0][1]],
                [OTHER[0][2]]]) #grab first state we have w/orientation too

    #is it len(measurements) or len(other)?
    for n in range(len(OTHER)):

        # prediction
        x = (F * x) + u
        P = F * P * F.transpose()
        print type(OTHER[n][1])

        Z = matrix([[OTHER[n][1]], [OTHER[n][2]]])
        y = Z - (H * x) #x is initial step. //TODO - initialize
        S = H * P * H.transpose() + R
    ...
    xy_estimate = H * x #this is a guess
