...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #distance to perpendicular of a circle is the distance from center to pt, less the radius.

        if self.x <= radius:
            h = radius
            k = radius
            cte = sqrt(pow((self.x-h), 2) + pow((self.y-k), 2)) - radius
        elif self.x >= 3.0*radius:
            h = 3.0 * radius
            k = radius
            cte = sqrt(pow((self.x-h), 2) + pow((self.y-k), 2)) - radius
        else:
            # if the robot is in between the two circle ranges
            if self.y > radius:
                # use the top half
                cte = self.y - 2.0*radius
            else:
                # use the bottom half
                cte = -self.y #it needs to be negative because the robot must go left.
        #
        #
        return cte
...
radius = 25.0
params = [10.0, 15.0, 0]
err, path = run(params, radius, True)
print '\nFinal parameters: ', params, '\n ->', err

draw_track(radius,radius*2,path)
